...
public class HM10Activity extends Activity  {
	...
	public void sendATCommand(String ATcommand) {
		if ((mState == WSTATE_DUMMY)) {
			...
			mPreviousTextSent = mTextSent.getText().toString();
			nextState();
		} else
	...
	protected void nextState(){
			switch(mState) {			   
			   case (WSTATE_CONNECT):
			       if (Constants.DEBUG) Log.i("JMG", "State Connected");
			       mTumakuBLE.connect();
			       break;
			   case(WSTATE_SEARCH_SERVICES):
			       if (Constants.DEBUG) Log.i("JMG", "State Search Services");
				   mTumakuBLE.discoverServices();
			       break;			   
			   case(WSTATE_READ_KEY):
			       if (Constants.DEBUG) Log.i("JMG", "State Read Key");
			   	   mTumakuBLE.read(TumakuBLE.SENSORTAG_KEY_SERVICE,TumakuBLE.SENSORTAG_KEY_DATA);
				   break;
			   case(WSTATE_NOTIFY_KEY):
			       if (Constants.DEBUG) Log.i("JMG", "State Notify Key");
			   ...
			   case(WSTATE_WRITE_KEY):
				   String tmpString=mTextSent.getText().toString();
			   	   mTextSent.setText("");
			       if (Constants.DEBUG) Log.i("JMG", "State Write State " + tmpString);
    ...
    private class HM10BroadcastReceiver extends BroadcastReceiver {
       ...
       public void onReceive(Context context, Intent intent) {
           if (intent.getAction().equals(TumakuBLE.DEVICE_CONNECTED)) {
		       if (Constants.DEBUG) Log.i("JMG", "DEVICE_CONNECTED message received");
           ...
           if (intent.getAction().equals(TumakuBLE.DEVICE_DISCONNECTED)) {
		       if (Constants.DEBUG) Log.i("JMG", "DEVICE_DISCONNECTED message received");
           ...
           if (intent.getAction().equals(TumakuBLE.SERVICES_DISCOVERED)) {
		       if (Constants.DEBUG) Log.i("JMG", "SERVICES_DISCOVERED message received");
           ...
           if (intent.getAction().equals(TumakuBLE.READ_SUCCESS)) {
		       if (Constants.DEBUG) Log.i("JMG", "READ_SUCCESS message received");
           ...
           if (intent.getAction().equals(TumakuBLE.WRITE_SUCCESS)) {
		       if (Constants.DEBUG) Log.i("JMG", "WRITE_SUCCESS message received");
           ...
           if (intent.getAction().equals(TumakuBLE.NOTIFICATION)) {
		       ...
		       if (!notificationValue.equalsIgnoreCase("null")) {
		    	   if (characteristicUUID.equalsIgnoreCase(TumakuBLE.SENSORTAG_KEY_DATA)) {
		 		       if (Constants.DEBUG) Log.i("JMG", "NOTIFICATION of Key Service");
		 		       if (notificationValueByteArray==null) {
		 		    	  if (Constants.DEBUG) Log.i("JMG", "No notificationValueByteArray received. Discard notification");
		 		    	  return;
		 		       }
		    	   }
		       }
           ...
           if (intent.getAction().equals(TumakuBLE.WRITE_DESCRIPTOR_SUCCESS)) {
		       if (Constants.DEBUG) Log.i("JMG", "WRITE_DESCRIPTOR_SUCCESS message received");
